---
title: Resume
layout: page
---

# <center>불편함을 찾고, 해결하고 싶은 개발자<br>강준우입니다.</center>
<center></center>

[//]: # (<center>저는 <mark>이런 것을</mark> 좋아하는 사람입니다.</center>)

<br>

# Junwoo Kang, Software Engineer
---

### Contacts
&nbsp; 010-5495-4551 · [0119299@gmail.com](mailto:0119299@gmail.com) · [instargram](https://www.instagram.com/vwxy_h__s_/)

### 지원 직무
&nbsp; Junior Backend Engineer

### 보유 역량
&nbsp;**Backend Engineering**
- Spring framework, JPA or Query 기반으로 프로젝트용 Backend API 서버 개발
- AWS ElasticBeanstalk을 이용한 블루/그린 배포를 개인 블로그에 적용하여 배포한 적 있음.
- Docker Registry를 통한 Image 관리 및 Docker, Docker-Compose를 통한 서버 배포
<br> 
<br>

&nbsp;**GitHub 역량**

- 주어진 workflow를 완벽하게 이해할 수 있고 재현할 수 있음.
- git을 이용한 버전 관리 능력은 당장 협업에 뛰어들어도 문제 없음.
- GitHub Action을 이용하여 CI/CD를 개인 블로그에 구축함.
<br>
<br>

&nbsp;**커뮤니케이션 역량**

- 다수의 프로젝트, 동아리에서 리더의 역할을 수행함.
- 상황에 맞게 상대를 설득할 수 있는 여러 가지 말하기 방법을 알고 있음.
- 상대를 배려하며 의견을 효과적으로 전달하는 방법을 알고 있음.


# Careers

---








# Experiences

---

[//]: # ()
[//]: # (### PNU AID 동아리 운영진<br>_WEB Backend 담당 &#40;Django&#41;_)

[//]: # ("ERD 제작"을 통해 <mark>관계형 DB 구축</mark>에 도움을 주었음)

[//]: # ()
[//]: # ("BE 팀장"을 맡으며 <mark>팀원과 함께</mark> 성장함)

[//]: # ()
[//]: # (_2023.11.06 ~ 현재_)

[//]: # ()
[//]: # (<br>)


&nbsp; **PNU CSE 졸업과제(Smart Factory)-팀장**<br>
&nbsp;&nbsp;철강 불량 검출 및 분류 인공지능 학습 모델 개발


<u>은상 수상</u> (14 개팀) 및 학생 투표 1위

#### <u>은상 수상 (14 개팀)</u> 및 학생 투표 1위


"철강 촬영 App 제작"을 통해 <br>
촬영 및 데이터 전송을 가능하게 하는 App 개발(FE, BE)

<br>
<a href="https://github.com/June222/capstone-2023-1-02" target="_blank">"철강 데이터 관리 App"</a>
을 통해 <br>
<mark>공장 관리자의 현장 사용</mark>을 목표로 App 개발 (FE)

</div>

<a href="https://github.com/June222/capstone-2023-1-02" target="_blank">"철강 데이터 관리 App"</a>
을 통해 <br>
<mark>공장 관리자의 현장 사용</mark>을 목표로 App 개발 (FE)

<a href="https://private-user-images.githubusercontent.com/38900003/280456095-97e9af17-62f3-4761-83e8-ad175bda8376.gif?jwt=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..bXzgDQnHfW5KlI_pIqtMKjZmYhfCmyzawqGcz3k6pcE" target="_blank">"데이터의 그래프 시각화"</a>
를 통해 

<mark>데이터 시각화</mark> 그리고 <mark>데이터 활용 방법 제시</mark>

"SpringBoot DB(AWS) JPA 개발"을 통해

<mark>AWS를 활용하여 배포<mark> (BE)

_2023.10.01 ~ 2023.11.3_

<br>

### PNU 산업 인공지능 프로젝트 경진대회<br>_FE 제작 (Flutter)_
#### <u>우수상 수상 (본선 8개 팀)</u>
"App UI ver.1 제작"을 통해 <mark>ver.2로 발전할 수 있는 기틀을 마련</mark>함

<a href="https://github.com/June222/No-Crime-App" target="_blank">"현재 사용자 위치 범죄 안전도 예측"</a> 구현

"범죄 건수 조회" 구현

"다른 지역 범죄 안전도 예측" 구현

위 과정을 통해 <mark>UX에 대한 필요성</mark>을 느낌.

_2023.08.28 ~ 2023.09.08_

<br>

### 블루투스를 활용한 임베디드 시스템 실험 프로젝트<br>_SW 제작 (C Language)_
#### <u>최고 성적</u>
"<mark>알고리즘 개발</mark> 및 적용"에 대한 계획을 수립

아이디어를 제시하고 수정하며 <mark>효과적인 Communication의 필요성</mark>을 느낌.

_2022.12.01 ~ 2022.12.21_

<br>

# Tech Stack

---

|    Tech    | Stack                |
|:----------:|----------------------|
|  Backend   | - Spring<br>- Django |
|  Frontend  | - Flutter<br>- Figma |
|     DB     | - MySQL              |

<br>

# Education

---

2016.03.02 ~ 현재 : 부산대학교 정보의생명공학부 정보컴퓨터공학과 재학

<br>

# Other Experience

---

|         기간          |        프로그램명        | 프로그램 내용                                                                                                                                                                          |
|:-------------------:|:-------------------:|:---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    23.11.03 ~ 현재    |     AID 동아리 운영진     | - Django 이용한 백엔드 서버 제작<br>- ERD 제작                                                                                                                                               |
| 23.09.03 ~ 23.12.19 |  PNU JAVA 실습 수업 조교  | - JAVA 실습 수업<br>- 채점 자동화 프로그램 작성                                                                                                                                                 |
| 23.09.23 ~ 23.12.02 |     백엔드 미니 부트캠프     | - Dispatcher Servlet<br>- DI / IoC<br>- Controller<br>- Persist Context<br>- TestCode 작성법<br>- linux 명령어<br>- Docker 사용법<br>- AWS 배포<br>- EC2 사용법<br>- ElasticBeanstalk 블루/그린 배포 |
| 23.01.01 ~ 23.03.15 | 부산대 맛집 어플 개발| Flutter 와 Firebase를 활용한 Side Project|
| 22.09 01 ~ 22.10.08 | ICPC Seoul Regional | 알고리즘 대회 준비                                                                                                                                                                       |

<br>

# About Me

---

|   MY   |                                     Link                                      |
|:------:|:-----------------------------------------------------------------------------:|
| github | <a href="https://github.com/june222" target="_blank"> github.com/june222 </a> |
| Email  |           <a href="mailto:0119299@gmail.com">0119299@gmail.com</a>            |
|  Blog  |              <a href="june222.github.io"> june222.github.io </a>              | 

<br>